...
//	List<Recommandation> listerRecomFilterAvis(@Param("reccat") RecommandationCategory reccat,@Param("recavis") RecommandationAvis recavis);
	@Query("Select rec From Recommandation rec where rec.recomCategory = :reccat AND rec.country = :patrie")
	List<Recommandation> listerRecomFilterPays(@Param("reccat") RecommandationCategory reccat,@Param("patrie") String patrie);
}
